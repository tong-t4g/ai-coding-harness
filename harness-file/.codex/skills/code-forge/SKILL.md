...
project_profile 不会直接传给 openspec-propose 或 writing-plans（它们有自己的上下文读取机制），但 CodeForge 在调用它们之前，会将 profile 信息作为**上下文前缀**拼接到 args 中：
...
项目分析完成后，检测知识图谱工具的可用性。知识图谱是**可选增强**：不可用时 CodeForge 仍能完整运行，各阶段自动回退到 grep/Read。
...
**编排层约束**：知识图谱逻辑只在 propose.md/apply.md/archive.md 中以「优先使用 XX 工具，不可用时回退」的形式出现，SKILL.md 只做可用性检测与状态记录，不引入复杂 KG 逻辑。CodeForge 自有的逻辑（archive 规格一致性验证、propose 计划质量审查）是**显式增强**，不伪装为「纯透传」——定位是「轻量编排框架」而非「零自有逻辑的纯编排」。
...
- **通用规则（适用于所有 checkpoint）**：如果有活跃变更目录且 `superpowers/plans/` 下有对应的计划文件（含 `<!-- codeforge change: <active_change> -->`），说明实际进度已进入 apply 阶段，修正 `phase: apply`，根据 checkbox 状态判定 checkpoint
...
**phase = apply 时验证：**
- `superpowers/plans/` 下是否有包含 `<!-- codeforge change: <active_change> -->` 的计划文件？ → 无则回退到 propose 阶段
...
| 有计划文件？ | `superpowers/plans/` 下是否有与活跃变更对应的计划文件（文件含 `<!-- codeforge change: <name> -->`） | 有 → 看实现状态 |
