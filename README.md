...
要处理下，写完计划后，用户确认时，怎么修改需求。要求需求修改后，openspec change 和 实现计划同步修改
状态管理和断点恢复，要不要提取到单独的文件中。考虑到模型是有智能的，是不是不应该定义那么详细的路由规则？

验证下 openspec 的指令是否和 skill 的行为完全一致
